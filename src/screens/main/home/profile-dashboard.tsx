...
import type { UserProfile, DailyProfileData, LocalizedHeaderProps } from './types';

const LocalizedHeader = ({ user }: LocalizedHeaderProps) => {
    ...
    return (
        <View style={styles.header}>
            <AppText variant="caption1" style={styles.date} color="light-gray">
                {formattedDate}
            </AppText>
            <AppText variant="subtitle1" color="white">
                {t('profileDashboard.goodDay')},{' '}
                {user?.full_name || t('profileDashboard.guest')}
            </AppText>
        </View>
    );
};
        ...
        <TouchableOpacity
            ...
            <View style={styles.profileContent}>
                ...
                <AppText style={styles.subtitle} variant="subtitle1" color="white">
                    {t('profileDashboard.todayScore')}
                </AppText>
                <View style={styles.scoresRow}>
                    <CircularScore
                        value={data?.today_wealth_points}
                        type="wealth"
                        title={t('dailyProfileDetail.wealth')}
                    />
                    <CircularScore
                        value={data?.today_study_points}
                        type="learning"
                        title={t('dailyProfileDetail.learning')}
                    />
                    <CircularScore
                        value={data?.today_relationship_points}
                        type="relation"
                        title={t('dailyProfileDetail.relation')}
                    />
                    <CircularScore
                        value={data?.today_career_points}
                        type="career"
                        title={t('dailyProfileDetail.career')}
                    />
                </View>
            </View>
        </TouchableOpacity>
